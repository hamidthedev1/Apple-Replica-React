...
function Footer() {
   useEffect(() => {
     const handleResize = () => {
       ...
       if ($(window).width() <= 768) {
         $(".footer-links-wrapper").addClass("expanded");
       } else {
         $(".footer-links-wrapper").removeClass("expanded");
         $(".footer-links-wrapper ul").show(); // Ensure all lists are shown on desktop
       }
     };

     // Handle the initial window size
     handleResize();

     // Attach resize event listener
     $(window).on("resize", handleResize);

     // Attach click event listener for footer links
     $(document).on("click", ".expanded h3", function () {
       const $this = $(this);
     ...
     return () => {
       ...
       $(document).off("click", ".expanded h3");
     };
   }, []);
  return (
    <>
